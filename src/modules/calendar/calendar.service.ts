...
export class CalendarService {
   ...
   * Deletes the CalendarToken so the user can re-authenticate with the correct account.
   */
  async disconnectProvider(userId: string, provider: 'google' | 'microsoft') {
    const deleted = await this.tokenModel.findOneAndDelete({ userId, provider });

    if (!deleted) {
      throw new NotFoundException(`No ${provider} calendar connection found for this user`);
    }

    this.logger.log(`Disconnected ${provider} calendar for userId=${userId} (was: ${deleted.microsoftEmail || 'N/A'})`);
